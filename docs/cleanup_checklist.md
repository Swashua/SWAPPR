...
- [ ] **`output.txt`**: Delete this temporary output file from the root directory.
- [ ] **`desktop.ini`**: Remove instances of `desktop.ini` from the root and `public/` directories (and ensure they are gitignored if using Windows).
...
## 5. Update `.gitignore`
- [ ] Ensure `.gitignore` is updated with:
  ```gitignore
  output.txt
  desktop.ini
  SWAPPR PICS/
