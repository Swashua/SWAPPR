...
// Output shape and sort order. department defaults to "" when the row carries
// no department_reserved.
assert.deepStrictEqual(out, [
  { code: "CIS 1201", description: "PROGRAMMING II", department: "" },
  { code: "CS 3106N", description: "INFORMATION ASSURANCE AND SECURITY", department: "" },
]);

// department_reserved is cleaned onto each subject (single-value normalizer,
// same canonical form as the batch cleanDepartments).
const deptRows = [
  {
    course_code: "CS 1101",
    course_description: "INTRO TO COMPUTING",
    department_reserved: "DCISM - SOME UNIT",
  },
  {
    course_code: "CS 1102",
    course_description: "PROGRAMMING I",
    department_reserved: "DEPARTMENT OF ACCOUNTANCY",
  },
];
const deptOut = filterSubjects(deptRows, ["CS"]);
assert.strictEqual(
  deptOut.find((s) => s.code === "CS 1101").department,
  "Department of Computer, Information Sciences and Mathematics",
  "DCISM abbreviation expands",
);
assert.strictEqual(
  deptOut.find((s) => s.code === "CS 1102").department,
  "Department of Accountancy",
  "dirty uppercase dept is title-cased",
);

// cleanDepartmentName normalizes a single raw value.
assert.strictEqual(
  cleanDepartmentName("DEPARTMENT OF ACCOUNTANCY"),
  "Department of Accountancy",
);
assert.strictEqual(cleanDepartmentName(""), "");
assert.strictEqual(cleanDepartmentName(null), "");
